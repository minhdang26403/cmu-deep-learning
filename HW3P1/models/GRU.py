...
class GRU:
    def __init__(self, input_size, hidden_size, num_layers=1):
        ...
        for i in range(num_layers):
            input_shape = NotImplemented                          # TODO : input_size if i == 0 else hidden_size 
            gru_cell = GRUCell(input_shape, NotImplemented)       # TODO : what should be passed to the second argument?
    ...
    def forward(self, input_seq, h0=None):
        ...
        for l_i, cell in enumerate(self.layers):
            ...
            h0_i = NotImplemented       # TODO : refer the appropriate index from h0
            h_prev = h0[h0_i]    
            for t in range(seq_len):
                x_t = out[t]            # shape: (batch_size, input_dim)
                new_h = NotImplemented  # TODO : add zeros of size (batch_size, hidden_size)
                ...
                for b in range(batch_size):
                    ...
                    h_new = cell.forward(NotImplemented, NotImplemented)
                    new_h[b] = h_new
                ...
                h_prev = NotImplemented # TODO : not that new_h
            self.caches[l_i] = layer_cache
            self.hiddens.append(new_h)  # final hidden state for this layer

            # Prepare input for next layer
            out = np.stack(layer_output, axis=0)    # now shape (seq_len, batch_size, hidden_size)
        
        h_n = np.stack(self.hiddens, axis=0)        # shape: (num_layers, batch_size, hidden_size)
        return out, h_n
    ...
    def backward(self, d_out):
        ...
        for l_i, cell in enumerate(reversed(range(self.num_layers))):
            layer_cache = self.caches[l_i]
            d_h_next = NotImplemented # TODO zeros array of size batch_size, self.hidden_size))
            d_input_layer = NotImplemented # TODO np.zeros((seq_len, batch_size, cell.Wrx.shape[1]))
            ...
            for t in reversed(range(seq_len)):
                ...
                d_x_t = NotImplemented          # TODO : make a zeros array (think about what will be the shape)
                d_h_prev = NotImplemented       # TODO : make a zeros array (think about what will be the shape)
                for b in range(batch_size):
                    # Restore the cell's internal variables.
                    # TODO : read appropriate values from the cache
                    cache = layer_cache[t][b]
                    cell.x, cell.hidden, cell.r, cell.z, cell.n = NotImplemented, NotImplemented, NotImplemented, NotImplemented, NotImplemented
                    
                    # Call the cell's backward.
                    dx, dh_prev = cell.backward(NotImplemented) # TODO : send the right batch from d_h
                    d_x_t[b] = NotImplemented                   # TODO : think of the appropriate value to be used here
                    d_h_prev[b] = NotImplemented                # TODO : think of the appropriate value to be used here
                
                d_input_layer[t] = d_x_t                        # pass gradient for input time step
                d_h_next = d_h_prev  # pass gradient to previous time step
            d_layer = NotImplemented # TODO : think about what needs to be propagated backwards from the GRU layers
