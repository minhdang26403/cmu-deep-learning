...
# Helper functions
# =============================================================================
def load_solution_data(filename="solution_outputs.npz"):
...
def test_gru():
    ...
    if np.allclose(student_d_input, solution_d_input, rtol=1e-5, atol=1e-7):
        ...
        mean_diff, rel_err, max_diff = compute_error_metrics(student_d_input, solution_d_input)
        debug_lines.append(
...
def test_train():
    ...
    try:
        ...
        if not hasattr(train_module, "train"):
            raise AttributeError("Module 'train_end2end' does not have a 'train' function.")
    ...
    debug_lines.append(f"Initial loss: {initial_loss:.4f}, Final loss: {final_loss:.4f}")
...
def run_all_tests():
    ...
    report_lines = []
    report_lines.append("Problem: GRU (Score: {} / {})".format(gru_result["score"], gru_result["max_score"]))
    if gru_result["feedback"]:
        report_lines.append("Feedback:")
        report_lines.append(gru_result["feedback"])
    report_lines.append("-" * 40)
    report_lines.append("Problem: train_end2end (Score: {} / {})".format(train_result["score"], train_result["max_score"]))
...
def main(args):
    ...
    print("train_end2end: {} / {}".format(results["train_end2end"]["score"], results["train_end2end"]["max_score"]))
